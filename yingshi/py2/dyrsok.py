...
class Spider(Spider):
    ...
    # ===== 播放器 (含 cookie 的智能地址处理) =====
    def playerContent(self, flag, id, vipFlags):
        """播放地址处理: 直链(https://)直接返回, /api/m3u8拼接主机名并跟随302"""
        play_id = id.split("$",1)[1] if "$" in id else id
        if not play_id: return {"parse":0, "url":"", "header":{}}
        ...
        if 'play_url=' in play_id:
            ...
            if raw_url.startswith('http://') or raw_url.startswith('https://'):
                return {"parse":0, "url": raw_url,
                        "header":{"User-Agent":self.headers['User-Agent'], "Referer":self.host+"/"}}
            else:
                api_url = self.host + (raw_url if raw_url.startswith('/') else '/'+raw_url)
                try:
                    # 使用带 cookie 的 session 请求，确保超级线路/王者TV蓝光认证通过
                    r = self.session.get(api_url, allow_redirects=True,
                        headers={'Referer': self.host+'/', 'User-Agent': self.headers['User-Agent']},
                        timeout=10)
                    if r.status_code == 200 and r.text.strip():
                        if '#EXTM3U' in r.text:
                            return {"parse":0, "url": api_url,
                                    "header":{"User-Agent":self.headers['User-Agent'], "Referer":self.host+"/"}}
                        try:
                            data = r.json()
                ...
                except Exception as e: print(f"API解析失败: {e}")
                return {"parse":0, "url": api_url,
                        "header":{"User-Agent":self.headers['User-Agent'], "Referer":self.host+"/"}}

        if not play_id.startswith('http'):
            play_id = urljoin(self.host, play_id)
